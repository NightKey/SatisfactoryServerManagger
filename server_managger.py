...
class managger:
    steam_update_command = command(
        "steamcmd +force_install_dir {path} +login {user_data} +app_update 1690800 validate +quit")
    ...
    linux_run_before = command(
        "chmod +x {path}/Engine/Binaries/Linux/UE4Server-Linux-Shipping")
    linux_run_command = command(
        "{path}/Engine/Binaries/Linux/UE4Server-Linux-Shipping FactoryGame {additionals}")

    def __init__(self, server_path: str = None, steam_username: str = None, steam_password: str = None, logger: Logger = None, additionals: str = None) -> None:
        ...
        logger.info(
            f"Server managger created with server path: {self.server_path}")

    def _update_server(self) -> bool:
        self.logger.info("Updating server")
        update_command = managger.steam_update_command.fill(
            user_data=self.user_info, path=self.server_path)
        self.logger.info(
            f"Update called with following data: {update_command}")
        try:
    ...
    def start_server(self, update_before: bool = False) -> bool:
        ...
        if self.is_running:
            self.logger.info(
                "Server start called, while the server is already running!")
            return True
        self.run = True
        if update_before:
            self._update_server()
        ...
        start_command = self.environment_specific_command.fill(
            path=self.server_path, additionals=self.additionals)
    ...
    def stop_server(self) -> None:
        ...
        while self.server.poll() is None:
            self.logger.warning(
                f"Serve did not terminate after {sleep_count*60} secunds!")
            if sleep_count >= 2:
