...
def start(msg: smdb_api.Message) -> None:
    ...
    if started:
        ...
        loop_thread.start()
    api.send_message(
        "Server started successfully." if started else "Server start failed!", msg.sender)
...
def update(msg: smdb_api.Message) -> None:
    ...
    api.send_message(("Server restarted successfully!" if managger.loop_running else "Server updated successfully!")
                     if updated else "Server update failed!", msg.sender)
...
def restart(msg: smdb_api.Message) -> None:
    ...
    api.send_message(
        "Server restarted successfully!" if restart else "Server update failed!", msg.sender)


def is_running(msg: smdb_api.Message) -> None:
    api.send_message(
        "The server is up and running" if managger.is_running else "The server is stopped", msg.sender)
